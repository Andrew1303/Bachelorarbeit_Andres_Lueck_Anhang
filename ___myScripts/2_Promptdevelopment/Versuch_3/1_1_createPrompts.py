...
# This script is used to check the quality of the prepared data.
# There were found some issues:
# - Couldnt iterate over the fourth place of the list, cause it did'nt exist.
# -> Corrected by adding empty strings for description of values, where it was missed and empty warning field

# for item in items:
#     if item[3]:
#         print(item[3])
    # # Name to fill into the prompt:
    # print(item[0])
    # # Expected Values
    # print(item[1])
...
promptlistpath = ""
for part in pathpartlist:
    promptlistpath = os.path.join(promptlistpath, part)
...
promptlistpath = ""
for pathpart in pathpromptlist:
    promptlistpath = os.path.join(promptlistpath, pathpart)
...
promptlistpath = ""
for part in promptlistpathlist:
    promptlistpath = os.path.join(promptlistpath, part)
...
savepathlist = ["___myScripts", "2_Promptdevelopment", "Versuch_3", "GeneratedPromptsWithEmbeddings", f"{name}"]
savepath = ""
