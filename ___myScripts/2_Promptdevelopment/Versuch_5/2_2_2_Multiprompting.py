...
Modelcombination = [
    [falcon3models, llamamodels],
    [falcon3models, deepseekmodels],
    [falcon3models, gemmamodels],
    [falcon3models, qwen3models],
    [qwen3models, deepseekmodels],
    [qwen3models, gemmamodels],
    [qwen3models, qwen3models],
    [qwen3models, falcon3models]
]
...
for mc, modelcombination in enumerate(Modelcombination):
    counter = 0
    results = []
    print(f"{mc+1}/{len(Modelcombination)}")
    generationmodel = modelcombination[0]
    evamodellist = modelcombination[1]
    maxlength = amount_prompts*iterations*len(generationmodel)*len(evamodellist)
    progresbar(counter, maxlength)
    for em, evamodel in enumerate(evamodellist):
        # For each model (5)
        for m, model in enumerate(generationmodel):
            model = model["name"]
            # For each language (1)
            for language in prompts:
                # for each prompt (55)
    ...
    name = name.replace(".", "_")
    name = name.replace(":", "_")
    savepathlist = ["___myScripts", "2_Promptdevelopment", "Versuch_5", "results_massproduction", f"{name}.json"]
    savepath = ""
